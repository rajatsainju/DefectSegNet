...
#                        required = True)
    ...
    parser.add_argument('--verbose', 
                        help='whether or not to print dev scores each iteration', 
                        default = False)
    ...
    parser.add_argument('--sqlite_dbase',
                        help = 'The database file for sqlite',
                        type = str,
                        default = 'sqlite:///test_mlrad_db.db')

    parser.add_argument('--num_configs',
                        help = 'The numbre of configurations to test',
                        type = int,
                        default = 100)
...
def update_args_choco(args, config):
...
def init_choco_sampler(args):  
   ...
   space = {"lr": choco.log(low = -5, high = -3, base = 10),
...
def main():
   ...
   sampler,conn = init_choco_sampler(args_0)
   mb = args_0['segment_size']
...
#     l = lines[i]
   for i in range(args_0['num_configs']): #NOTE make this a meaningful unhardcoded number
     token, params = sampler.next()
     args = update_args_choco(args_0.copy(), params)
     print('executing')
     ...
     mean_val_iu = np.mean([val_stats['iu_scores'][t] for t in targets])
     sampler.update(token, 1. - mean_val_iu)
     if mean_test_iu > best_iu:
        best_iu = mean_test_iu
        best_time_stamp = time_stamp
        print("NEW BEST IU%s"%(best_iu))
   all_results = conn.results_as_dataframe()
   print(all_results)
